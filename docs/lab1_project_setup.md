...
- Project name: `Ch17_AdvanceCourse` (또는 원하는 영문 이름)
...
L1 단계에서는 Voice 임포트 **건너뛰고** Step 6 (폴더 구조 정리) 으로 바로 진행.
...
`Assets/` 루트에 다음 폴더들을 만든다. **번호 prefix 로 학습 흐름과 맞춤** (Project 창에서 알파벳 정렬 시 학습 순서대로 나열):

```
Assets/
├─ 1.Scenes/                    ← 베이스 씬 (MainScene.unity)
├─ 2.Scripts/
│  ├─ 1.Voice/                  ← L2·L3·L4 (Voice·PTT·Spatial Audio)
│  ├─ 2.Chat/                   ← L5
│  ├─ 3.Emote/                  ← L6
│  ├─ 4.Pose/                   ← L7
│  ├─ 5.Movement/               ← L8·L9 (보간·Blend·IK·시선)
│  ├─ 6.Social/                 ← L10
│  └─ 7.UI/                     ← L11
├─ 3.Prefabs/
├─ 4.Materials/
...
심화 모듈에서 만드는 산출물이 학습 흐름 순서대로 각 폴더에 모인다 (1.Scenes → 2.Scripts → 3.Prefabs ...). 학생 제출 시 `1.Scenes ~ 5.Animations` 폴더 통째로 zip.

### 6-2. URP 템플릿 기본 자료 정리 (선택, 권장)

Unity 6.3 URP 템플릿이 자동 생성한 다음 항목들은 학습에 불필요하니 삭제 권장:

| 항목 | 이유 |
|---|---|
| `Assets/TutorialInfo/` (+ `.meta`) | URP 튜토리얼 자료 |
| `Assets/Readme.asset` (+ `.meta`) | URP 템플릿 readme |
| `Assets/Scenes/` 폴더 통째 (`SampleScene.unity` 포함) | 우리는 `Assets/1.Scenes/MainScene.unity` 를 만들 거임 |

**유지** 할 것:
- `Photon/`, `QuantumUser/` — Quantum SDK 와 사용자 코드
- `Settings/` — URP 렌더 설정 (필수)
- `TextMesh Pro/` — TMP (UI 학습에 필요)
- `InputSystem_Actions.inputactions` — Unity Input System (입력 학습)

> ⚠️ Unity Editor **닫고** 외부 탐색기에서 삭제 권장 — Editor 열린 채로 외부 변경하면 `.meta` 충돌 가능. Unity 안에서 우클릭 Delete 도 OK.
...
`Ctrl+S` → `Assets/Scenes/MainScene.unity` 로 저장.
...
- [ ] `Assets/` 하위에 폴더 구조 만들어짐
- [ ] `Assets/Scenes/MainScene.unity` 가 빈 씬에서 정상 실행
- [ ] Console 에 빨간 에러 없음

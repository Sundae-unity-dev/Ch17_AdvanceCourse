...
`Assets/Interaction/` 폴더 생성. **번호 prefix 로 학습 흐름과 맞춤** (Project 창에서 알파벳 정렬 시 학습 순서대로 나열):
...
| `Assets/Scenes/` 폴더 통째 (`SampleScene.unity` 포함) | 우리는 `Interaction/1.Scenes/MainScene.unity` 를 만들 거임 |
...
📸 **L1_10.png** — 정리 완료된 `Assets/` 최상위 구조 (Interaction · Photon · QuantumUser · Settings · TextMesh Pro 만 남은 상태)

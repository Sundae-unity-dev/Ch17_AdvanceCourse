...
📸 **L9_04** — LeftFootIK GameObject + TwoBoneIK 컴포넌트
...
📸 **L9_05** — Two Bone IK Inspector (Root/Mid/Tip 채워짐)
...
📸 **L9_06** — FootIKController.cs 코드 (UpdateFootTarget)

### 3-2. 프리팹 부착

PlayerCharacter 에 `FootIKController` 추가, Inspector 에 모든 슬롯 연결.

📸 **L9_07** — FootIKController Inspector 필드 입력

### 3-3. 경사면 테스트

씬에 경사진 평면 만들기 (Plane 회전 15도) → 캐릭터가 경사 위에서 발이 평면에 붙음.

📸 **L9_08** — 경사면 위 캐릭터, 발이 자연스럽게 평면에 닿음
...
📸 **L9_09** — HeadAim + MultiAimConstraint 컴포넌트
...
📸 **L9_10** — MultiAimConstraint Inspector
...
📸 **L9_11** — LookAtController.cs 코드 (FindNearestPlayer + Update)

### 5-2. 프리팹 부착·테스트

부착 후 두 명 가까이 다가가면 머리만 자연스럽게 상대를 향함.

📸 **L9_12** — 가까운 플레이어 바라보는 캐릭터
...
📸 **L9_13** — 뒤쪽 플레이어가 있어도 시선 weight = 0 (Inspector 확인)
